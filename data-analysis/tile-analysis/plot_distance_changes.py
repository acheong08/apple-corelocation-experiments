...
def main():
    ...
    plt.title('Average Distance Moved Over Time\n(Batched by 1000 Location Changes)', fontsize=14)
    ...
    print(f"Overall average distance: {overall_avg:.1f} m")
    print(f"Min distance: {min(distances):.1f} m")
    print(f"Max distance: {max(distances):.1f} m")
    print(f"Standard deviation: {np.std(distances):.1f} m")
